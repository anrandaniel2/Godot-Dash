...
@export var override_mouse_filters: bool = false:
	set(val):
		override_mouse_filters = _set_override_mouse_filters(val)
...
func _ready() -> void:
	if not ScrollDebugger.debug_gradient:
		ScrollDebugger.setup_debug_drawing()
...
func _draw() -> void:
	if debug_mode:
		ScrollDebugger.draw_debug(self)
...
func scroll(vertical: bool, axis_velocity: float, axis_pos: float, delta: float) -> void:
	...
	if is_zero_approx(delta):
		if vertical:
			...
			if get_v_scroll_bar():
				get_v_scroll_bar().set_value_no_signal(-axis_pos)
		else:
			...
			if get_h_scroll_bar():
				get_h_scroll_bar().set_value_no_signal(-axis_pos)
		return
...
func scroll_x_to(x_pos: float, duration := 0.5) -> void:
	if not should_scroll_horizontal():
		return
	if input_handler.content_dragging:
		return
...
func ensure_control_visible_smooth(control: Control) -> void:
	if not content_node:
		return
	if not content_node.is_ancestor_of(control):
		return
	if not scroll_damper:
		return
